...
impl HJBInventoryController {
    ...
    pub fn optimal_skew(&self, position: f64, max_position: f64) -> f64 {
        ...
        let terminal_skew_capped = terminal_skew.abs().min(
            diffusion_skew.abs() * self.config.max_terminal_multiplier
...
mod tests {
    ...
    fn test_hjb_zero_position_zero_skew() {
        ...
        assert!(skew.abs() < 1e-6, "Zero position should give ~zero skew: {}", skew);
    }
    ...
    fn test_hjb_long_position_positive_skew() {
        ...
        assert!(skew > 0.0, "Long position should give positive skew: {}", skew);
    }
    ...
    fn test_hjb_short_position_negative_skew() {
        ...
        let skew = ctrl.optimal_skew(-0.5, 1.0);
        assert!(skew < 0.0, "Short position should give negative skew: {}", skew);
    }
    ...
    fn test_hjb_funding_ewma() {
        ...
        assert!(rate2 > rate1, "EWMA should increase toward target: {} -> {}", rate1, rate2);
    ...
    fn test_hjb_value_gradient() {
        ...
        assert!(
            grad_zero != grad_long,
            "Gradient should depend on position"
        );
    }
