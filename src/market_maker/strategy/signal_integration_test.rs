...
mod signal_improvement_proof {
    ...
    impl ElevatedFlowScenario {
        fn new(steps: usize) -> Self {
            Self { steps, base_edge_bps: 3.0 }
        }
    }

    impl MarketScenario for ElevatedFlowScenario {
        fn n_steps(&self) -> usize { self.steps }

        fn state_at(&self, step: usize, position: f64, wealth: f64) -> StateSnapshot {
            let flow_intensity = (step as f64 / self.steps as f64 * std::f64::consts::PI * 2.0).sin().abs();
            StateSnapshot {
                position,
                wealth,
        ...
        fn name(&self) -> &str { "ElevatedFlowScenario" }
    }
    ...
    impl HighVolScenario {
        fn new(steps: usize) -> Self { Self { steps } }
    }
    ...
    impl MarketScenario for HighVolScenario {
        ...
        fn name(&self) -> &str { "HighVolScenario" }
    }
        ...
        fn solve(&self, state: &dyn ControlStateProvider, params: &MarketParams) -> ControlOutput {
            ...
            if params.is_toxic_regime {
                return ControlOutput {
                    action: Action::NoQuote { reason: NoQuoteReason::CascadeDetected },
                    expected_value: 0.0,
                    confidence: 0.2,
                };
            }
            ...
            let ladder = Ladder {
                bids: smallvec![LadderLevel { price: 100.0, size: 1.0, depth_bps: bid_depth.max(2.0) }],
                asks: smallvec![LadderLevel { price: 100.0, size: 1.0, depth_bps: ask_depth.max(2.0) }],
            };
    ...
    impl EnhancedSolver {
        fn new() -> Self {
            Self { informed_adj: InformedFlowAdjustment::default() }
        }
    }
    ...
    impl ControlSolver for EnhancedSolver {
        fn solve(&self, state: &dyn ControlStateProvider, params: &MarketParams) -> ControlOutput {
            ...
            let should_pull = params.is_toxic_regime
                || (state.confidence() < 0.4 && p_informed > 0.3);

            if should_pull {
                return ControlOutput {
                    action: Action::NoQuote { reason: NoQuoteReason::HighUncertainty },
                    expected_value: 0.0,
                    confidence: 0.1,
                };
            }
    ...
    fn test_regime_kappa_provides_value() {
        ...
        println!("Delta:     {:.2} bps PnL, {:.3} Sharpe", delta_pnl, delta_sharpe);
    ...
    fn test_combined_improvements() {
        ...
        println!("║ {:<25} {:>10} {:>10} {:>10} ║", "Scenario", "Base", "Enhanced", "Δ PnL");
        println!("╠══════════════════════════════════════════════════════════════════╣");
        ...
        println!("╠══════════════════════════════════════════════════════════════════╣");
        println!("║ {:<25} {:>10} {:>10} {:>+9.2} ║", "AVERAGE", "", "", avg_delta);
        println!("╚══════════════════════════════════════════════════════════════════╝");
    ...
    fn test_spread_calculation_differences() {
        ...
        println!("{:<20} {:>10} {:>15} {:>15}", "Condition", "Kappa", "Half-Spread", "Total Spread");
        println!("{}", "-".repeat(65));

        // Baseline: fixed kappa
        let kappa_baseline = 2000.0_f64;
        let spread_baseline = (1.0 / gamma) * (1.0 + gamma / kappa_baseline).ln() * 10000.0;
        println!("{:<20} {:>10.0} {:>14.2}bp {:>14.2}bp",
            "Baseline (any)", kappa_baseline, spread_baseline, spread_baseline * 2.0);
        ...
        for (name, kappa) in [
            ...
            println!("{:<20} {:>10.0} {:>14.2}bp {:>14.2}bp", name, kappa, spread, spread * 2.0);
        }
        ...
        println!("\n=== VERIFICATION ===");
        println!("In high vol (kappa=800 vs 2000): spread widens from {:.1}bp to {:.1}bp",
    ...
    fn test_signal_integration_components_work() {
        ...
        assert!(mid_mult >= 0.95 && mid_mult <= 1.05, "Should be near 1.0 for moderate informed");
        ...
        assert!(spread_low_kappa > spread_high_kappa * 2.0,
        ...
        println!("  Ratio: {:.2}x wider", spread_low_kappa / spread_high_kappa);
    }
}
