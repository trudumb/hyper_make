...
mod tests {
    ...
    fn test_market_params() -> MarketParams {
        MarketParams {
            sigma: 0.01, // 100 bps/sqrt(s) vol — large enough for self-consistent gamma
            sigma_effective: 0.01,
            kappa: 5000.0,
            microprice: 100.0,
            tail_risk_intensity: 0.0, // Default calm
            ..Default::default()
        }
    }
    ...
    fn test_strategy() -> GLFTStrategy {
        let cfg = RiskConfig {
            ...
            min_spread_floor: 0.00001, // 0.1 bps — lets natural gamma control spread
            ..Default::default()
        };
    ...
    fn test_strategy_with_log_odds(use_log_odds: bool, max_bps: f64) -> GLFTStrategy {
        let risk_config = RiskConfig {
            use_log_odds_as: use_log_odds,
            max_as_adjustment_bps: max_bps,
            // Low floor so self-consistent gamma doesn't dominate in unit tests
            min_spread_floor: 0.00001,
            ..Default::default()
        };
    ...
    fn test_monopolist_pricing_not_applied_with_competitors() {
        ...
        let risk_config_off = RiskConfig {
    ...
    fn test_monopolist_markup_capped() {
        let risk_config = RiskConfig {
            use_monopolist_pricing: true,
            monopolist_markup_cap_bps: 3.0, // Tight cap
            ..Default::default()
        };
        let strategy = GLFTStrategy::with_config(risk_config);
        let config = test_quote_config();
